<template>
    <section>
        ...
        >
            ...
            <inertia-link
                v-if="userCan('announcement.create')"
                class="
                    button button-default-responsive button-primary
                    flex flex-row items-center
                "
                :href="$route('admin.edu.announcement.create')"
            >
        ...
        <div class="bg-white py-6 shadow-subtle rounded-lg">
            ...
            >
                <div class="w-full md:w-1/3">
                    <input-group
                        input-autocomplete="announcement_name_search"
                        input-class="form-control form-control-short"
                        input-id="announcement_name"
                        input-name="announcement_name"
                        input-placeholder="Announcement Name"
                        input-type="text"
                        :label-hidden="true"
                        label-text="Announcement Name"
                        v-model="editableSearchOptions.announcement_name"
                    />
                </div>
            </div>
            ...
            <template v-else>
                        ...
                        >
                            ...
                            <td v-if="showActions">
                                <div class="flex flex-row items-center justify-end -mx-1">
                                    <button
                                        v-if="userCan('announcement.publish')"
                                        class="
                                    ...
                                    <inertia-link
                                        v-if="userCan('announcement.edit')"
                                        class="
                                            flex flex-row items-center inline-flex mx-1 px-2 py-1 rounded text-theme-base-subtle-contrast text-sm tracking-wide
                                            focus:outline-none focus:ring
                                            hover:bg-theme-info hover:text-theme-info-contrast
                                        "
                                        :href="$route('admin.edu.announcement.edit', item.id)"
                                        title="Edit"
                                    >
                                        <icon-edit
                                            class="w-4"
                                    ...
                                    <button
                                        v-if="userCan('announcement.delete')"
                                        class="
                                            flex flex-row items-center inline-flex mx-1 px-2 py-1 rounded text-theme-base-subtle-contrast text-sm tracking-wide
                                            focus:outline-none focus:ring
                ...
                >
                    <pagination :pagination="announcement.pagination" />
                </div>
            </template>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            showPagination() {
                try {
                    return this.announcement.pagination.last_page > 1;
                } catch (e) {
                    return false;
                }
            },
            showActions() {
                return this.userCan('announcement.edit') || this.userCan('announcement.delete');
            },
            announcementData() {
                if (!this.announcement || !this.announcement.data || this.announcement.data.length < 1) {
                    return false;
                }

                return this.announcement.data;
            }
        },
        methods: {
            ...
            confirmPublish() {
                ...
                this.$inertia.patch(
                    this.$route('admin.edu.announcement.publish', this.itemToPublish.id),
                    {
                        only: [
                            'flash',
                            'announcement'
            ...
            confirmDelete() {
                ...
                this.$inertia.delete(
                    this.$route('admin.edu.announcement.destroy', this.itemToDelete.id),
                    {
                        only: [
                            'flash',
                            'announcement'
            ...
            onSearchOptionsUpdate: _.debounce(function () {
                ...
                router.get(
                    this.$route('admin.edu.announcement.index'),
